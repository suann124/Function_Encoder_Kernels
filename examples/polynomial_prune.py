...
class TrainPruneAnalyzer:
    ...
    def identify_redundant_basis(self, 
        ...
        # # Align basis functions to important PCs
        # basis_alignment = np.zeros((n_basis, n_components))
        # for i in range(n_basis):
        #     for j in range(n_components):
        #         basis_alignment[i, j] = self.cos_similarity(
...
if __name__ == "__main__":
    ...
    # np.savez('results/pca_results_poly.npz', 
    #      eigenvalues=eigenvalues, 
    #      eigenvectors=eigenvectors, 
    ...
    # comparison_to_save = {
    #     'mse_original': comparison_results['mse_original'],
    #     'mse_pruned': comparison_results['mse_pruned'],
    #     'mse_pruned_refined': comparison_results['mse_pruned_refined'],
    #     'y_pred_original': comparison_results['y_pred_original'].cpu().numpy(),
    #     'y_pred_pruned': comparison_results['y_pred_pruned'].cpu().numpy(),
    #     'y_pred_pruned_refined': comparison_results['y_pred_pruned_refined'].cpu().numpy(),
    #     'coeffs_original': comparison_results['coeffs_original'].cpu().numpy(),
    #     'coeffs_pruned': comparison_results['coeffs_pruned'].cpu().numpy(),
    #     'coeffs_pruned_refined': comparison_results['coeffs_pruned_refined'].cpu().numpy(),
    # }
    # np.savez('results/comparison_results_poly.npz', **comparison_to_save)

    # # Save data for function approximation plot (4th plot)
    # np.savez('results/function_approximation_poly.npz',
    #         X_sorted=X_sorted,
    #         y_sorted=y_sorted,
    ...
    # # Save data for coefficient comparison plot (5th plot)
    # np.savez('results/coeff_comparison_poly.npz',
    ...
    # import json
    # summary = {
    #     "mse_original": float(comparison_results['mse_original']),
    #     "mse_pruned": float(comparison_results['mse_pruned']),
    #     "mse_pruned_refined": float(comparison_results['mse_pruned_refined']),
    #     "n_basis_original": len(original_model.basis_functions.basis_functions),
    #     "n_basis_pruned": len(pruned_model.basis_functions.basis_functions),
    #     "n_basis_pruned_refined": len(pruned_model_refined.basis_functions.basis_functions),
    #     "compression_ratio": len(pruned_model_refined.basis_functions.basis_functions) / len(original_model.basis_functions.basis_functions),
    #     "performance_ratio": comparison_results['mse_pruned_refined'] / comparison_results['mse_original'],
    # }
    # with open('results/performance_summary_poly.json', 'w') as f:
