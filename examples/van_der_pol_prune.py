...
class VanDerPolPruneAnalyzer:
    ...
    #     print("Using cosine similarity between PCs and basis functions to identify important basis functions...")
    #     if X is None:
    #         # Create sample Van der Pol inputs following polynomial pattern
    #         # Single initial state and time step for all points
    #         y0 = torch.tensor([[1.0, 0.0]], device=self.device)  # [1, 2] - single initial state
    #         dt = torch.tensor([[0.1]], device=self.device)  # [1, 1] - single time step
    #         X = (y0, dt)
